...
define([
    ...
    "N/redirect",
    "./Lib/rxrs_util",
], function (render, record, file, search, redirect, rxrsUtil) {
    var FOLDER_MRR = 809;
    var mrr_id;
    ...
    function getMrrFolder(id) {
        try {
            var lookupRs = search.lookupFields({
                type: rxrsUtil.getReturnRequestType(id),
                id: id,
           ...
           ['parent', 'anyof', [PARENT_FOLDER_ID]]*/,
            ...
            log.error("getMrrFolder", e.message);
        }
    }
    ...
    function getReturnRequestFolder(rrName, rrId, mrrFolderId) {
        try {
            ...
            log.error("getReturnRequestFolder", e.message);
        }
    }
    ...
    function generatePdf(context) {
        try {
            ...
            log.debug(
                "generatePdf",
                "Generate for 2FRN " + request.parameters.custscript_kd_2frn_id
            );
            try {
                ...
                var rId = f2rnRec.getValue("custrecord_kd_returnrequest");
                var rrType = rxrsUtil.getReturnRequestType(rId);
                var lookupRs = search.lookupFields({
                    type: rrType,
                    id: rId,
                    columns: ["custbody_kd_master_return_id", "tranid", "internalid"],
                });
                log.debug("lookupRs", lookupRs);

                var rrTranId = lookupRs["tranid"];
                var rrId = lookupRs["internalid"][0].value;
                log.debug(
                    "generatePdf",
                    "lookup MRR ID: " + lookupRs["custbody_kd_master_return_id"][0].value
                );
                ...
                pdf.folder = getReturnRequestFolder(
                    rrTranId,
                    rrId,
                    getMrrFolder(f2rnRec.getValue("custrecord_kd_returnrequest"))
                );
                ...
                if (searchResultCount > 0 && searchResultCount == form222FileCount) {
                    ...
                    var isTrackingNumberGenerated = taskRec.getValue(
                        "custevent_kd_tracking_num"
                    );
                    if (isTrackingNumberGenerated == true) {
                        log.debug("tracking number is generated");
                        if (rrRec.getValue("transtatus") == rxrsUtil.rrStatus.C2Kittobemailed) {
                            log.debug("RR Rec Status is C2Kit to be mailed");
                            //rrRec.setValue({fieldId:'transtatus',value: 'D'});
                            log.debug("rrType",rrType)
                            try {
                                record.submitFields({
                                    type: rrType,
                                    id: rrId,
                                    values: {
                                        transtatus: rxrsUtil.rrStatus.PendingPackageReceipt,
                                    },
                                });
                            } catch (e) {
                                log.error("Error in updating RR Status", e.message)
                            }
